...
### 6.1 Became live in Phase 2 stage 3 (2026-08-05)

Browser automation shipped, so the web-content threats modelled above are now reachable rather than anticipated. Two decisions taken on 2026-08-05 **increase** the residual risk, and are recorded here as increases rather than folded into the mitigations that partly offset them.

| Threat | Why it is live now | Accepted residual risk |
|---|---|---|
| Prompt injection via a web page driving a logged-in browser (T-001 class) | `youtube.search` and `youtube.play` read and act on real pages, in the owner's own signed-in Brave profile (ADR-0019 Option D) | Accepted, with the controls stated: results are addressed **by position only** — no method resolves an element from page text, asserted structurally in `tests/security/test_prompt_injection.py` — page content enters a prompt only through `Observation`, with the delimiters in observed content escaped and left visible, and an anti-bot challenge stops the session rather than being worked around. What is *not* mitigated: a page that induces an action the owner asked for still executes as the signed-in user. |
| **Permission-scope creep on browser automation (T-030 class), widened by ADR-0032** | `browser.automate_logged_in` may now hold a standing `ALWAYS` grant, so automation can run with no per-action confirmation | **Accepted as a reduction in control, by explicit owner decision on 2026-08-05 after the trade-off was put to them in writing.** The mitigations are: it is offered rather than defaulted (the capability still evaluates to `ASK` until the owner chooses "Allow always"); it is an ordinary grant, visible on the Permissions screen and revocable there; the code default names no capability at all, so this is one line of `config/defaults.yaml` and removing it restores §11.1's posture. What is *not* mitigated: while the grant stands, the only thing between a hostile page and an action is the positional-selection control and the untrusted-content boundary. Those were already load-bearing; they are now more so. **Revisit trigger:** any measured instance of a page influencing which element an action lands on. |
| Closing the owner's browser as a side effect of automation | `browser.restart` (ADR-0032) can close every browser window | Accepted, and bounded by design: the browser is *asked* to close (`WM_CLOSE`) and never terminated, so Chromium saves its session and the tabs return; a window that refuses — a page confirming an unsaved change — produces a declared failure rather than an escalation, so the owner's unsaved work is never discarded to satisfy an automation step. It carries its own capability and its own approval, so it can be denied without denying browser automation as a whole. |
