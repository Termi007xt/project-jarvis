...
| Emergency stop | PRD §11.3 | Implemented (Phase 1) | `JarvisCore.emergency_stop` cancels tasks, releases locks, stops workers, denies any pending approval, and reports exactly what was stopped. Reachable from the tray, the Home screen and the configurable global hotkey (`jarvis.runtime.hotkeys`). A hotkey another process already owns is reported as unavailable rather than silently absent. |
...
| Local text-to-speech, sensitive-output filtering | PRD FR-030–FR-034, §21 (Phase 1) | Implemented (Phase 1) | `jarvis.audio.tts`. Kokoro `bm_george` verified on this machine. Secret-shaped text is replaced with a description of what it was **before** synthesis (FR-034), and every voice carries licence metadata marked non-redistributable while ADR-0014 is unresolved (FR-032). |
| Full-duplex audio and barge-in | PRD FR-015, ADR-0028 | Partial (Phase 1) | `jarvis.audio.duplex`. Playback never stops capture; detections during playback are judged against what Jarvis emitted; barge-in stops speech and releases no locks and cancels no tasks; the half-duplex degradation path is implemented. **Self-trigger rates have not been measured on real hardware**, which ADR-0028 requires before barge-in is relied upon. |
