...
## 14. Phase 0 security status

This table states, honestly and conservatively, what security-relevant controls exist in this repository. It was verified against the committed code and a passing test suite (386 tests, 89% statement coverage), not against the target design.
...
| Test harness | PRD §21, NFR-042 | Implemented | 386 tests across `tests/unit`, `tests/security`, `tests/integration`, `tests/ui` and `tests/acceptance`; 89% statement coverage. Every test gets an isolated vault; none requires Ollama, audio, a GPU or the network. |
...
| Approval port (deny by default) | PRD §11.2, ADR-0010 | Partial | The interface exists and the Phase 0 default **denies**: with no approval dialog, nothing requiring approval can run. The dialog itself is the first Phase 1 deliverable. |
...
| Emergency stop | PRD §11.3 | Partial | `JarvisCore.emergency_stop` cancels tasks, releases locks, stops workers and reports exactly what was stopped; reachable from the tray and the Home screen. The **global hotkey** is Phase 1. |
| Wake phrase, VAD, ring buffer, push-to-talk | PRD FR-010–FR-018, §21 (Phase 1) | Deferred (Phase 1) | Not in Phase 0 scope. |
| Local speech recognition (STT), no raw-audio retention | PRD FR-020–FR-025, §21 (Phase 1) | Deferred (Phase 1) | `faster-whisper` is specified as the provider in `config/defaults.yaml`; not yet integrated into a running application. |
| Local text-to-speech, TTS worker isolation | PRD FR-030–FR-039, FR-038, §21 (Phase 1) | Deferred (Phase 1) | Exploratory, standalone voice scripts exist under `tools/voice-lab` and `tools/qwen-tts-lab` as research spikes; these are not part of the product runtime, are not covered by this policy's controls, and must not be treated as a security-reviewed component. |
...
| Secrets storage (DPAPI / Windows Credential Manager) | PRD §18.3, NFR-022 | Deferred (Phase 1) — ADR-0008 | No secret ever needs to be stored before an integration requiring one ships; this must land no later than the first feature that requests a credential, which may be earlier than Phase 6 in practice. |
