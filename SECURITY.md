...
| Wake phrase, VAD, ring buffer, push-to-talk | PRD FR-010–FR-018, §21 (Phase 1) | Implemented (Phase 1) | The pipeline, ring buffer, VAD and push-to-talk are implemented; the pre-wake buffer has **no file-handling code at all**, so FR-012 and AT-002 are structural. Tests: `tests/acceptance/test_at002_wake_privacy.py`. Wake detection is active on the pretrained openWakeWord "Hey Jarvis" model, installed by `--install-wake-model` and never bundled; user acceptance on 2026-08-04 reported **no false wakes** over an extended period of ordinary conversation. Bare "Jarvis" is not detected (0 of 4 measured) and the GUI says so rather than implying it works (FR-011). Per-user enrolment and its measurement are still not built, and always-listening is a switch the user turns on rather than a default. |
...
| Browser restart is an approved act, never a forced kill | ADR-0032, ADR-0010 | Implemented (Phase 2 stage 3) | `browser.restart` posts `WM_CLOSE` to the browser's visible windows and polls until the process exits, so Chromium saves its session and the tabs return; a browser that will not close is a declared failure and never escalates to `TerminateProcess`. Its own capability and its own approval — approving a search is not approving the loss of the user's windows. No new process-creation call site: closing is `user32.PostMessageW` and reopening goes through `launch_argv`, so ADR-0029 is untouched. Tests: `tests/unit/test_browser_restart_and_tabs.py`. |
| An offered approval scope is one the engine can grant | PRD §9.9, ADR-0027, ADR-0032 | Implemented (Phase 2 stage 3) | **Regression.** The dialog offered "Allow for this task" for conversation-driven requests, which carried no `task_id`; the engine rejected the scope and the user's choice was discarded, silently, on every utterance — four times in seven minutes on 2026-08-05, visible in `audit.jsonl` as `approval scope 'task' rejected`. A conversation turn now carries a per-turn task id, and `tests/unit/test_approval_scopes_that_stick.py` walks the offered-scope table and grants each entry so the two cannot drift apart again. |
| Standing grant for one named medium-risk capability | PRD §9.9, §11.1, ADR-0032 | Implemented (Phase 2 stage 3) | `DefaultPolicy.always_allowable_capabilities` permits an `ALWAYS` grant for capability ids named one at a time; `config/defaults.yaml` names only `browser.automate_logged_in`. **Offered, never defaulted** — it still evaluates to `ASK` until the owner chooses it, and it is then an ordinary revocable grant. The code default is empty, so a fresh install keeps §11.1's posture exactly. High risk is never eligible: that check runs first and does not consult the list. Accepted as a **reduction in control**, recorded in `THREAT_MODEL.md` rather than presented as neutral. Tests: `tests/unit/test_approval_scopes_that_stick.py`. |
| Browser teardown actually runs | ADR-0031, ADR-0032 | Implemented (Phase 2 stage 3) | **Regression.** Playwright's sync API is bound to its creating thread; sessions were created on a tool-pool worker and closed from the GUI thread, so every recorded session ended with `greenlet.error: Cannot switch to a different thread` — meaning the browser was not closed and the debugging port was left open. Teardown is a security control under ADR-0031, so a teardown that always raised was the control not running. `BrowserWorkspace` now owns one thread and routes every browser touch through it. Tests: `tests/unit/test_browser_restart_and_tabs.py`. |
